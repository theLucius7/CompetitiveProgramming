...
void solve() {
	...
	int n;
	cin >> n;
	vector<vector<array<int, 2>>> a(n), b(n);
	vector<int> cnt(n);
	...
	// for (int i = 0; i < n; i ++ ) {
	// 	cout << a[i].size() << " ";
	// 	for (int j = 0; j < a[i].size(); j ++ ) {// ==UserScript==
...
(function(){
	...
	// }
	for (auto &[_, vv] : v) {
		sort(vv.begin(), vv.end(), [&](array<int, 2> x, array<int, 2> y) {
			return x[0] > y[0];
		});
	}
	for (auto [x, y] : mp) {
		...
		if (c == 0) {
			...
			for (int j = 0; j < b[i].size(); j ++ ) {
				...
				for (int k = vv.size() - 1; k >= 0; k -- ) {
					if (vv[k][0] <= y) {
						auto it = s.lower_bound({cnt[vv[k][1]], vv[k][1]});
						s.erase(it);
						cnt[vv[k][1]] -- ;
						s.insert({cnt[vv[k][1]], vv[k][1]});
						vv.pop_back();
					}
				}
			}
